...
export async function POST(request: Request) {
  ...
  const response = await openai.createChatCompletion({
    model: "gpt-3.5-turbo", // change to 'gpt-4' if have access to it
    temperature: 0.8,
    n: 1,
    stream: false,
    messages: [
      {
        role: "system",
        content:
          "when responding, welcome the user always as MR.Husain and say welcome to the Todo App! Limit the response to 200 characters",
      },
      {
        role: "user",
